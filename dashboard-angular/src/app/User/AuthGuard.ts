...
export class AuthGuard implements CanActivate {
    ...
    canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
      ...
      if (userRole === 'Guest' && this.isRestrictedRoute(state.url)) {
        this.router.navigate(['']);
        return false;
      }
      if (userRole === 'Employee' && this.isRestrictedRouteEmp(state.url)) {
        this.router.navigate(['/login']);
        return false;
      }
    ...
    private isRestrictedRouteEmp(url: string): boolean {
      // Check if the route starts with '/dashboard/', '/admin/', or '/component/'
      return url.startsWith('/dashboard/userlist');
    }
}
